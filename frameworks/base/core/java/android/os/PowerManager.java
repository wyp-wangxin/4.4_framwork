...
public final class PowerManager {
     ...
     * </p>
     */
     //如果持有该类型的wakelock锁，则按Power键灭屏后，即使允许屏幕、按键灯灭，也不会释放该锁，
     //CPU不会进入休眠状态
     ...
     */
     //如果持有该类型的wakelock锁，则使屏幕保持亮/Dim的状态，键盘灯允许灭，按Power键灭屏后，会立即释放
    @Deprecated
     ...
     * as the user moves between applications and doesn't require a special permission.
     */
     //如果持有该类型的wakelock锁，则使屏幕保持亮的状态，键盘灯允许灭，按Power键灭屏后，会立即释放
    @Deprecated
     ...
     * as the user moves between applications and doesn't require a special permission.
     */
     //如果持有该类型的wakelock锁，则使屏幕、键盘灯都保持亮，按Power键灭屏后，会立即释放
    @Deprecated
     ...
     * {@hide}
     */
     //如果持有该锁，则当PSensor检测到有物体靠近时关闭屏幕，远离时又亮屏，
     //该类型锁不会阻止系统进入睡眠状态，比如
     ...
     * </p>
     */
     //在释放锁时，如果wakelock带有该标志，则会小亮一会再灭屏，
