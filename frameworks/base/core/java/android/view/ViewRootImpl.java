...
public final class ViewRootImpl implements ViewParent,
    ...
    /*wwxx
	App与WMS的连接，首先会建立一个Session到WMS，之后就会通过IWindowSession接口与WMS中的Session直接通信，
	IWindowSession类是什么？它指向了一个实现了IWindowSession接口的Session代理对象。
	当应用程序进程启动第一个Activity组件的时候，它就会请求WMS服务发送一个建立连接的Binder进程间通信请求。
	WMS服务接收到这个请求之后，就会在内部创建一个类型为Session的Binder本地对象，
	并且将这个Binder本地对象返回给应用程序进程，App就会得到一个Session代理对象，
	并且保存在ViewRootImpl类的成员变量mWindowSession中

	当App有了这个远端Session代理对象mWindowSession之后，所有向WMS的请求都通过mWindowSession来进行。
	举例来说：ViewRootImpl要添加窗口，就使用mWindowSession代理对象的addToDisplay方法调用到远端Session对象的addToDisplay方法
    ...
    */
    ...
    public ViewRootImpl(Context context, Display display) {
        ...
        mWindow = new W(this);
		/*
		W对象是什么？ViewRootImpl::W：用于向WMS提供接口，让WMS控制App端的窗口。
		
		它可看作是个代理，很多时候会调用ViewRootImpl中的功能。

		W的声明中有两个成员变量：mViewAncestor和mWindowSession，它一头连着App端的ViewRootImpl，
		一头连着WMS中的Session，且实现了IWindow的接口。意味着它是App和WMS的桥梁，是WMS用来回调App端，
		让ViewRootImpl做事用的。
            ...
            if (mView == null) {
                ...
                try {
                    ...
                    res = mWindowSession.addToDisplay(mWindow, mSeq, mWindowAttributes,
                            ...
                            mAttachInfo.mContentInsets, mInputChannel);//这行代码大部分人都知道这个是与WMS做IPC通信的
                } catch (RemoteException e) {
                    mAdded = false;
                    mView = null;
	...
	/*
	看看requestLayout方法，首先检验是不是主线程在修改UI，
	然后 调用scheduleTraversals，在scheduleTraversals开始执行了，
	才会有我们熟悉的onDraw,onLayout,onMeasure
	*/
    @Override
