...
public final class WindowManagerGlobal {
        ...
        synchronized (WindowManagerGlobal.class) {
            if (sWindowSession == null) {
                try {
                    ...
                    //wwxx 调用WMS的openSession创建一个Session对象
	...
	//wwxx 第一件事情：检查参数是否合法，如果是子Window,还要调整布局参数
    public void addView(View view, ViewGroup.LayoutParams params,
        ...
        synchronized (mLock) {
			...
			//第二件事情	:创建ViewRootImpl，将View添加到列表中，
			//这里每次都会new一个对象，所以说调用一次addView，就会有一个ViewRootImpl。
			...
			3. 负责管理、布局和渲染应用程序窗口视图的UI。*我们在看ViewRootImpl的setView 方法。
