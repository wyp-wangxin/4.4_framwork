...
public final class ActivityStackSupervisor {
    ...
    final int startActivityLocked(IApplicationThread caller,
            Intent intent, String resolvedType, ActivityInfo aInfo, IBinder resultTo,/*wwxx参数resultTo是Launcher这个Activity里面的一个Binder对象，
		...
		/*wwxx 接下来，创建即将要启动的Activity的相关信息，并保存在r变量中*/
    ...
    final int startActivityUncheckedLocked(ActivityRecord r,
        ...
        int launchFlags = intent.getFlags();/*函数首先获得intent的标志值，保存在launchFlags变量中*/
        ...
        if (((launchFlags&Intent.FLAG_ACTIVITY_NEW_TASK) != 0 &&
            ...
            if (r.resultTo == null) {
                ...
                /* wwxx 我们分析的是launcher 启动activity（app）这段代码的逻辑是查看一下，
                  当前有没有Task可以用来执行这个Activity。
                  由于r.launchMode的值不为ActivityInfo.LAUNCH_SINGLE_INSTANCE，因此，
                  它通过findTaskLocked函数来查找存不存这样的Task，
        ...
        if (r.packageName != null) {
            ...
            有些情况下，如果即将要启动的Activity就在堆栈的顶端，那么，
            就不会重新启动这个Activity的别一个实例了。现在处理堆栈顶端的Activity是Launcher，
            与我们即将要启动的MainActivity不是同一个Activity，
            因此，这里不用进一步处理上述介绍的情况。*/
